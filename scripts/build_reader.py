...
def main() -> None:
    ...
    shutil.copytree(READER, DIST / "client")
    (DIST / "server").mkdir(parents=True)
    (DIST / "server" / "index.js").write_text(
        """const worker={async fetch(request,env){const url=new URL(request.url);let path=url.pathname;if(path==="/"||!path.split("/").pop().includes("."))path="/index.html";let response=await env.ASSETS.fetch(new Request(new URL(path,request.url),request));if(response.status===404&&(request.headers.get("accept")||"").includes("text/html"))response=await env.ASSETS.fetch(new Request(new URL("/index.html",request.url),request));return response;}};export default worker;\n"""
    )
    (DIST / "server" / "package.json").write_text('{"type":"module"}\n')
    (DIST / ".openai").mkdir(parents=True)
    shutil.copy2(ROOT / ".openai" / "hosting.json", DIST / ".openai" / "hosting.json")
    print(f"Built reader/ and dist/ with {len(assets)} offline assets.")
