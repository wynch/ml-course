"""Exercise 1 — write the two lines that make the dual SVM work.
...
Fill in both TODOs. The checks at the bottom verify the KKT conditions of the
fitted SVM — support vectors exactly on the margin, nothing inside it, and
‖w‖² = Σαᵢ, which only holds at the true optimum.

Run:  cd python && uv run ../exercises/ex1_dual_solver.py
"""
...
def dual_gradient(alpha, Q):
    ...
    # TODO(you): one line. Differentiate the two terms and add them.
...
def project_feasible(v, y):
    ...
    """
    # TODO(you): bracket the root, bisect ~200 times, return the clipped vector.
    raise NotImplementedError
