"""Exercise 3 — make the spike appear, then move it.

The classical story says "more parameters ⇒ more overfitting". It is wrong past
a very specific point, and the point is easy to name: p = n, where the model has
exactly as many parameters as it has training examples.

Two TODOs:

  1. ``fit_min_norm`` — least squares when the design matrix is tall, and the
     *minimum-norm* interpolant when it is wide. One numpy call does both.
  2. ``peak_degree``  — run the sweep and return the degree with the worst test
     error.

Then answer the question the script asks at the end: the peak sits at degree
n − 1 for every n. Change ``N_TRAIN`` from 20 to 30 and check that it moves.

Run:  cd python && uv run ../exercises/ex3_double_descent.py
"""
...
def fit_min_norm(x, y, degree):
    ...
    """
    # TODO(you): build the design matrix and return the pseudo-inverse solution.
    raise NotImplementedError


def peak_degree(x, y, xt, yt, degrees):
    """The degree whose test MSE is largest — the interpolation threshold."""
    # TODO(you): fit at every degree, score on (xt, yt), return the worst degree
    # and the list of test MSEs.
    raise NotImplementedError
